...
使用方式：
- 交互式模式：python master_controller_v2.py
- 命令行模式：python master_controller_v2.py --exchanges binance bybit --top_n 100 --start_date 2025-07-01 --end_date 2025-07-09 --strategy 1

V2.0 更新：
- 升級到 calculate_FR_return_list_v3.py (數據完整性檢查版本)
- 升級到 strategy_ranking_v3.py (數據完整性檢查版本)
- 支持 --use-legacy 參數回退到 v2 版本
- 增強錯誤處理和數據完整性檢查
- 保持向後兼容性
...
- 錯誤隔離：單個交易對失敗不影響其他
- 部分成功保存：成功的結果立即保存
- 自動依賴檢查：strategy_ranking_v3 會自動檢查 return_metrics 依賴
"""
...
class MasterControllerV2:
    ...
    def run_complete_process(self, exchanges: List[str], top_n: int, start_date: str, end_date: str, strategy: str, args=None):
        ...
        if args and not args.no_telegram:
            ...
            message = f"master_controller_v2 開始執行{mode_info}\n開始時間: {start_time_utc}"
            self.send_telegram_notification(message)
        
        print("\n🚀 開始執行完整的資金費率分析流程 (V2.0)")
        print("=" * 60)
        ...
        for i in range(len(self.steps)):
            ...
            if not success:
                ...
                if args and not args.no_telegram:
                    ...
                    mode_info = " (舊版兼容模式)" if use_legacy else " (V3 數據完整性檢查)"
                    message = f"master_controller_v2 執行失敗{mode_info}\n失敗時間: {end_time_utc}"
                    self.send_telegram_notification(message)
        ...
        if args and not args.no_telegram:
            ...
            mode_info = " (舊版兼容模式)" if use_legacy else " (V3 數據完整性檢查)"
            message = f"master_controller_v2 執行完成{mode_info}\n完成時間: {end_time_utc}\n總耗時: {elapsed_minutes:.1f}分鐘"
            self.send_telegram_notification(message)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='資金費率分析系統總控程式 V2.0',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
使用範例:
  python master_controller_v2.py --exchanges binance bybit --top_n 100 --start_date 2025-07-01 --end_date 2025-07-09 --strategy 1
  python master_controller_v2.py --exchanges binance bybit --top_n 500 --start_date 2025-07-01 --end_date 2025-07-09 --strategy original
  python master_controller_v2.py --exchanges binance bybit --top_n 1000 --start_date 2025-07-01 --end_date 2025-07-09 --strategy all
  python master_controller_v2.py --exchanges binance bybit --top_n 100 --start_date up_to_date --end_date up_to_date --strategy 1
  python master_controller_v2.py --exchanges binance bybit --top_n 750 --start_date up_to_date --end_date up_to_date --strategy all --yes
  python master_controller_v2.py --exchanges binance bybit --top_n 750 --start_date up_to_date --end_date up_to_date --strategy all --yes --no-charts
  python master_controller_v2.py --exchanges binance bybit --top_n 750 --start_date up_to_date --end_date up_to_date --strategy all --yes --use-legacy

V2.0 新功能:
- 升級到 calculate_FR_return_list_v3.py 和 strategy_ranking_v3.py
- 自動數據完整性檢查和智能增量處理
- 使用 --use-legacy 參數可回退到 v2 版本
...
- V3 版本會自動檢查數據完整性並只處理缺失的數據
    ...
    # 創建控制器
    controller = MasterControllerV2()
    
    print("🎛️  資金費率分析系統總控程式 V2.0")
    print("=" * 50)
