...
5. V3版本改進：在統計資訊框中新增ROI顯示
6. V4版本改進：在圖表標題中添加時間起訖標記 (YYYY-MM-DD to YYYY-MM-DD)
"""
...
class ReturnMetricsVisualizer:
    ...
    def create_return_charts(self, trading_pair, data, start_date=None, end_date=None):
        ...
        cumulative_title = f'{trading_pair} - Cumulative Return {time_range}'
        daily_title = f'{trading_pair} - Daily Return {time_range}'
        
        # 創建包含兩個子圖的figure
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 10))
        
        # 子圖1：累積收益 - V4.0: 使用新的標題格式
        ax1.plot(data['date'], data['cumulative_return'], 
                linewidth=2, color='#2E86AB', alpha=0.8)
        ax1.set_title(cumulative_title, 
...
def main():
    """主函數"""
    parser = argparse.ArgumentParser(description='生成交易對收益圖表 (V4版本)')
